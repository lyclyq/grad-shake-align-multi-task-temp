...
def _aligned_param(param: torch.Tensor, ref: torch.Tensor) -> torch.Tensor:
    if param.dtype == ref.dtype and param.device == ref.device:
        return param
    return param.to(device=ref.device, dtype=ref.dtype)
...
class LoRALinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        lora_A = _aligned_param(self.lora_A, x_d)
        lora_B = _aligned_param(self.lora_B, x_d)
        lora = (x_d @ lora_A.t()) @ lora_B.t()
...
class DualRankLoRALinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        lora_A_r = _aligned_param(self.lora_A_r, x_d)
        lora_B_r = _aligned_param(self.lora_B_r, x_d)
        lora_r = (x_d @ lora_A_r.t()) @ lora_B_r.t()

        if self.hi > 0 and self.use_hi:
            lora_A_hi = _aligned_param(self.lora_A_hi, x_d)
            lora_B_hi = _aligned_param(self.lora_B_hi, x_d)
            lora_hi = (x_d @ lora_A_hi.t()) @ lora_B_hi.t()
        else:
            lora_hi = 0.0
